...
//        System.out.println(solution.findKthLargest(new int[]{2, 1}, 2));
        System.out.println(solution.findKthLargestByHeap(new int[]{3, 2, 1, 5, 6, 4}, 2));
    }
    ...
    class Solution {
        ...
        private int partition(int[] nums, int lo, int rh) {
            ...
            int r = rh;
            while (true) {
                while (nums[l] <= nums[lo] && l < rh) {
                    l++;
                }
                while (nums[r] >= nums[lo] && r > lo) {
                    r--;
                }
                if (l >= r) {
                    break;
        ...
        public int findKthLargestByHeap(int[] nums, int k) {
            //use minimum heap to maintain k large numbers
